...
function icon (type) {
  switch (type) {
    ...
    case 'Polled':
      return <svg className='w-6 h-6' fill='currentColor' viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg'>
...
export default function GameplayTable ({ title = '', type, data }) {
  return (
      ...
      <thead>
        {type === 'platforms'
          ? <tr>
            ...
            <th>
              {icon('Polled')}
              <span className="label-hint">Polled</span>
            </th>
            <th>
              {icon('Main')}
              <span className="label-hint">Main</span>
            </th>
            <th>
              {icon('Extended')}
              <span className="label-hint">Extended</span>
            </th>
            <th>
              {icon('Completionist')}
              <span className="label-hint">Completionist</span>
            </th>
            <th>
              {icon('Minimum')}
              <span className="label-hint">Minimum</span>
            </th>
            <th>
              {icon('Maximum')}
              <span className="label-hint">Maximum</span>
            </th>
          </tr>
          : <tr>
            {columns(type).map(name =>
              name
                ? <th key={name} data-column={name}>
                  {icon(name)}
                  <span className="label-hint" tabIndex="0">{name}</span>
                </th>
                : <th key={name}></th>
            )}
          </tr>
      ...
      <tbody>
        {type === 'platforms'
          ? data.map(platformData =>
            <tr key={platformData.platform}>
              <td>{platformData.platform}</td>
              <td>{platformData.count_comp}</td>
              <td>{toRelativeTime(platformData.comp_main)}</td>
              <td>{toRelativeTime(platformData.comp_plus)}</td>
              <td>{toRelativeTime(platformData.comp_100)}</td>
              <td>{toRelativeTime(platformData.comp_low)}</td>
              <td>{toRelativeTime(platformData.comp_high)}</td>
            </tr>
          )
          : rows(type).map(([row, polled, ...fields], index) =>
            <tr key={index}>
              <td>
                {icon(row)}
                <span className="label-hint" tabIndex="0">{row}</span>
              </td>
              <td>{data[polled] || '-'}</td>
              {fields.map(field =>
                <td key={field}>{data[field] ? toRelativeTime(data[field]) : '-'}</td>
              )}
            </tr>
          )
        }
